...
for m = Int64[5, 10, 20, 40]
    ...
    @time x, count, history = jacobi(A, b, x0)
    println("iterations: ", count)
    plot(history, show = true, label = "Jacobi method for \$m = "*string(m)*"\$")
    println("Press <ENTER> to continue: ")
    readline()

    println("Gauss Seidel method for m = " * string(m))
    @time x, count, history = gauss_seidel(A, b, x0)
    println("iterations: ", count)
    plot(history, show = true, label = "Gauss Seidel method for \$m = "*string(m)*"\$")
    println("Press <ENTER> to continue: ")
    readline()
    ...
    for w = 1.0 : 0.1 : 1.9
        println("w = " * string(w))
        @time x, count, history = sor(A, b, x0, w)
        println("iterations: ", count)
        plot(history, show = true, label = "SOR method for \$m = "*string(m)*"\$ \$w = "*string(w)*"\$")
        println("Press <ENTER> to continue: ")
        readline()
    end

    println("Conjugate Gradient method for m = " * string(m))
    @time x, count, history = conjugate_gradient(A, b, x0)
    println("iterations: ", count)
    plot(history, show = true, label = "Conjugate Gradient method for \$m = "*string(m)*"\$")
    println("Press <ENTER> to continue: ")
    readline()
end
